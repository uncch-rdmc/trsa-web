package edu.harvard.iq.dataverse.util.json;

import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.google.gson.JsonObject;
import edu.harvard.iq.dataverse.entities.Dataset;
import edu.harvard.iq.dataverse.entities.DatasetVersion;
import edu.harvard.iq.dataverse.export.JSONExporter;
import java.io.FileOutputStream;
import java.nio.file.Path;
import java.nio.file.Paths;
import java.util.logging.Level;
import java.util.logging.Logger;
import javax.json.JsonObjectBuilder;

public class JsonUtil {

    private static final Logger logger = Logger.getLogger(JsonUtil.class.getCanonicalName());

    /**
     * Make an attempt at pretty printing a String but will return the original
     * string if it isn't JSON or if there is any exception.
     */
    public static String prettyPrint(String jsonString) {
        try {
            com.google.gson.JsonParser jsonParser = new com.google.gson.JsonParser();
            JsonObject jsonObject = jsonParser.parse(jsonString).getAsJsonObject();
            Gson gson = new GsonBuilder().setPrettyPrinting().create();
            String prettyJson = gson.toJson(jsonObject);
            return prettyJson;
        } catch (Exception ex) {
            logger.info("Returning original string due to exception: " + ex);
            return jsonString;
        }
    }

    public static void printMinDatasetMetadata(DatasetVersion dsv) {
        javax.json.JsonObject datasetAsJson = (JsonPrinter.jsonAsDatasetDto(dsv)).build();
        Path JsonMetadataFilePath = Paths.get(dsv.getDataset().getFileSystemDirectory().toString(), dsv.getDataset().getIdentifier() + ".json");
        try (FileOutputStream jsonFileOutputStream = new FileOutputStream(JsonMetadataFilePath.toFile())) {
        JSONExporter jsonExporter = new JSONExporter();
        jsonExporter.exportDataset(dsv, datasetAsJson, jsonFileOutputStream);
            
        } catch (Exception e) {
            logger.log(Level.INFO, "exception happened={0}", e);
        }
    }
}
